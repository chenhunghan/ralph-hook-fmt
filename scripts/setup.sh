...
case "$OS-$ARCH" in
  ...
  Linux-aarch64)
    ...
    ;;
  *)
    echo "{\"continue\": true, \"systemMessage\": \"[ralph-hook-fmt] unsupported platform $OS-$ARCH\"}"
    exit 0
    ;;
esac
...
if curl -fsSL "$RELEASE_URL" | tar xz 2>/dev/null; then
  ...
  echo "{\"continue\": true, \"systemMessage\": \"[ralph-hook-fmt] binary installed ($LATEST_VERSION)\"}"
else
  echo "{\"continue\": true, \"systemMessage\": \"[ralph-hook-fmt] failed to download binary from $RELEASE_URL\"}"
fi
